import React from 'react';

function CompanyQA() {

    return (

        <div id="faq" className="faq-area-wrapper img-h-w-wrap sp-y-custom">
            <div className="img-half-width right bg-img-nr" data-bg="image/tp02/faq-1.png"></div>
            <div className="container">
                <div className="row">
                    <div className="col-lg-6">
                        <div className="faq-content-wrap">
                            <div className="section-title">
                                <h5>FAQ</h5>
                                <h4 className="font-weight-bold">常見問題</h4>
                            </div>
                            
                            <div className="accordion mt-lg-0 my-4 my-md-5" id="accordionFaq">
                                <div className="card">
                                    <div className="card-header">
                                        <h4 data-toggle="collapse" data-target="#collapseOne" aria-expanded="true">京典臻品 JD Classic</h4></div>
                                    <div id="collapseOne" className="collapse show" data-parent="#accordionFaq">
                                        <div className="card-body">
                                            <p>京典臻品JD Classic｜天然水晶礦石提供多元招財水晶開運方法最佳選擇，風水師推薦招財進寶盆、咬錢貔貅水晶、淨化磁場能量鎮宅之寶，水晶之王燕尾鈦晶手排提升 .</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="card">
                                    <div className="card-header">
                                         <h4 data-toggle="collapse" data-target="#collapseTwo">臻情．臻心．臻價值</h4></div>
                                    <div id="collapseTwo" className="collapse" data-parent="#accordionFaq">
                                        <div className="card-body">
                                            <p>京典臻品JD Classic｜傳承精神與成熟技術，提供親切自然的體貼服務，讓顧客永遠掛著心滿意足的微笑。 「臻情．臻心．臻價值」就是我們極致追求：與顧客締結如親人般地感情、秉持極致完滿的認真態度、致力於高度專業鑑賞。</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="card">
                                    <div className="card-header">
                                        <h4 data-toggle="collapse" data-target="#collapseThree">榮獲「第18屆台灣品質保證金像獎」殊榮</h4>
                                    </div>
                                    <div id="collapseThree" className="collapse" data-parent="#accordionFaq">
                                        <div className="card-body">
                                            <p>感謝各位客戶與企業朋友一路的支持與中華工商經貿科技發展協會的肯定，榮獲「第18屆台灣品質保證金像獎」殊榮，在此京典臻品、時易藝品、京璽晶品、京采水晶，致力開發不同業種與品牌，目前有專業進口經營零售批發,烏拉圭紫水晶洞、巴西紫晶洞,瑪瑙水晶聚寶盆,黃水晶洞,鈦晶洞,瑪瑙水晶洞,意象晶洞,變種晶洞,大型紫晶洞,水晶開口笑,紫晶鎮,愛心晶片,燕尾 ...</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )

}

export default CompanyQA;